<?php
/**
 * AQM Blog Post Feed Updater
 * 
 * Simple GitHub updater for the AQM Blog Post Feed plugin.
 * Designed to avoid conflicts with the AQM Sitemaps plugin.
 */
...
class AQMBPF_Updater {
    ...
    private function get_github_update_data($force_check = false) {
        // Check cache first
        $cache_key = 'aqmbpf_github_data_' . md5($this->username . $this->repository);
    ...
    public function maybe_reactivate_plugin() {
        ...
        if (get_transient('aqmbpf_reactivate')) {
            ...
            error_log('[AQMBPF UPDATER] Attempting reactivation on admin page load');
