...
class AQM_Blog_Post_Feed_Module extends ET_Builder_Module {
    ...
    public function get_fields() {
        return array(
            ...
            'use_first_paragraph' => array(
                'label'           => esc_html__('Use First Paragraph as Excerpt', 'aqm-blog-post-feed'),
                'type'            => 'yes_no_button',
                'options'         => array(
                    'on'  => esc_html__('Yes', 'aqm-blog-post-feed'),
                    'off' => esc_html__('No', 'aqm-blog-post-feed'),
                ...
                'default'         => 'off',
                'description'     => esc_html__('When enabled, uses the first paragraph of content as excerpt if no manual excerpt is set.', 'aqm-blog-post-feed'),
                'show_if'         => array('layout_type' => array('grid', 'featured_list')),
            ),
            'excerpt_type' => array(
                'label'           => esc_html__('Excerpt Limit Type', 'aqm-blog-post-feed'),
                'type'            => 'select',
                'options'         => array(
                    'words' => esc_html__('Words', 'aqm-blog-post-feed'),
                    'chars' => esc_html__('Characters', 'aqm-blog-post-feed'),
                ),
                'default'         => 'words',
                'description'     => esc_html__('Choose whether to limit excerpt by words or characters.', 'aqm-blog-post-feed'),
                'show_if'         => array('layout_type' => array('grid', 'featured_list')),
            ),
            'excerpt_limit' => array(
                'label'           => esc_html__('Excerpt Limit', 'aqm-blog-post-feed'),
                'type'            => 'range',
                'default'         => 60,
                'options'         => array(
                    'min'  => 10,
                ...
                'description'     => esc_html__('Maximum number of words or characters to display in excerpt.', 'aqm-blog-post-feed'),
                'show_if'         => array('layout_type' => array('grid', 'featured_list')),
            ),
    ...
    // Helper function to generate excerpts based on settings
    public function generate_excerpt($use_first_paragraph = 'off', $excerpt_type = 'words', $excerpt_limit = 60) {
        if (has_excerpt()) {
            // If a manual excerpt exists, use it
            $excerpt = get_the_excerpt();
        } else if ($use_first_paragraph === 'on') {
            // Get the content and extract the first paragraph
            ...
            if (!empty($matches[1])) {
                // Use the first paragraph content without HTML tags
                $excerpt = strip_tags($matches[1]);
            } else {
                // Fallback to content if no paragraph found
                $excerpt = strip_tags($content);
            }
        } else {
            // Use the content as fallback
            $excerpt = strip_tags(get_the_content());
        }
        
        // Apply limit based on type
        if ($excerpt_type === 'chars') {
            // Limit by characters
            if (mb_strlen($excerpt) > $excerpt_limit) {
                $excerpt = mb_substr($excerpt, 0, $excerpt_limit) . '...';
            }
        } else {
            // Default: limit by words
...
public function render($attrs, $render_slug, $content = null) {
        ...
        $excerpt_type = isset($this->props['excerpt_type']) ? $this->props['excerpt_type'] : 'words';
        $use_first_paragraph = isset($this->props['use_first_paragraph']) ? $this->props['use_first_paragraph'] : 'off';
        $read_more_text = $this->props['read_more_text'];
        // Background size set to cover
        $background_size = 'cover';
        $background_zoom = 'cover'; // No zoom effect with cover
        ...
        if ($posts->have_posts()) {
            while ($posts->have_posts()) {
                ...
                if ($layout_type === 'list') {
                    ...
                    $output .= '<div class="aqm-card-image" style="background-image: url(' . esc_url($thumbnail_url) . '); background-size: cover; background-position: center;"></div>';
                    ...
                    // Generate excerpt using the helper function
                    $excerpt = $this->generate_excerpt($use_first_paragraph, $excerpt_type, $excerpt_limit);
                    $output .= '<div class="aqm-post-excerpt" style="color:' . esc_attr($content_color) . '; font-size:' . esc_attr($content_font_size) . 'px; line-height:' . esc_attr($content_line_height) . 'em;' . $content_font_family_style . '">' . $excerpt . '</div>';
                ...
                $output .= '<div class="aqm-post-item" style="border-radius: ' . esc_attr($item_border_radius) . 'px; overflow: hidden; position: relative; ' . $post_height_style . ' background-image: url(' . esc_url($thumbnail_url) . '); background-size: ' . esc_attr($background_size) . '; background-position: center; transition: all 0.5s ease;">';
                ...
                // Generate excerpt using the helper function
                $excerpt = $this->generate_excerpt($use_first_paragraph, $excerpt_type, $excerpt_limit);
                $output .= '<p class="aqm-post-excerpt" style="color:' . esc_attr($content_color) . '; font-size:' . esc_attr($content_font_size) . 'px; line-height:' . esc_attr($content_line_height) . 'em;' . ($content_font_family ? ' font-family:' . esc_attr($content_font_family) . ';' : '') . '">' . $excerpt . '</p>';
                ...
                button.on("click", function() {
                    ...
                    $.ajax({
                        ...
                        data: {
                            ...
                            use_first_paragraph: "' . $use_first_paragraph . '",
