...
function aqm_generate_excerpt($use_first_paragraph = 'off', $excerpt_type = 'words', $excerpt_limit = 60) {
    if (has_excerpt()) {
...
function aqm_load_more_posts_handler() {
    ...
    $excerpt_type = isset($_POST['excerpt_type']) ? sanitize_text_field($_POST['excerpt_type']) : 'words';
    $use_first_paragraph = isset($_POST['use_first_paragraph']) ? sanitize_text_field($_POST['use_first_paragraph']) : 'off';
    ...
    if ($posts->have_posts()) {
        while ($posts->have_posts()) {
            ...
            if ($layout_type === 'list') {
                ...
                $html .= '<div class="aqm-list-item" style="margin-bottom: ' . esc_attr($post_bottom_margin) . 'px;">';
                ...
                $html .= '<div class="aqm-card-item" style="margin-bottom: ' . esc_attr($post_bottom_margin) . 'px;">';
                
                // Featured image section
                $html .= '<div class="aqm-card-image" style="background-image: url(' . esc_url($thumbnail_url) . '); background-size: cover; background-position: center;"></div>';
                ...
                $excerpt = aqm_generate_excerpt($use_first_paragraph, $excerpt_type, $excerpt_limit);
                $html .= '<div class="aqm-post-excerpt" style="color:' . esc_attr($content_color) . '; font-size:' . esc_attr($content_font_size) . 'px; line-height:' . esc_attr($content_line_height) . 'em;">' . $excerpt . '</div>';
                ...
                $html .= '<div class="aqm-post-item" style="border-radius: ' . esc_attr($item_border_radius) . 'px; overflow: hidden; position: relative; ' . $post_height_style . ' background-image: url(' . esc_url($thumbnail_url) . '); background-size: cover; background-position: center; transition: all 0.5s ease;">';
            ...
            // Generate excerpt using the helper function
            $excerpt = aqm_generate_excerpt($use_first_paragraph, $excerpt_type, $excerpt_limit);
            $html .= '<p class="aqm-post-excerpt" style="color:' . esc_attr($content_color) . '; font-size:' . esc_attr($content_font_size) . 'px; line-height:' . esc_attr($content_line_height) . 'em;">' . $excerpt . '</p>';
